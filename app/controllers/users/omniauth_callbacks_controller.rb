class Users::OmniauthCallbacksController < Devise::OmniauthCallbacksController
  ...
  def facebook
    if params[:state] && current_user
      if save_token('facebook')
        ...
        redirect_to edit_user_registration_url(current_user)+'?already_linked=true'
      end
  ...
  def store_omniauth_data
    if request.env["omniauth.auth"] #solo accedemos a los datos si los hay, a veces hay errores.
      ...
      if omniauth_data.provider == 'facebook'
        # (intentar) obtener una imagen de FB de unos 350 px de ancho
        omniauth_data.info.image = omniauth_data.info.image + '?width=350'
      end
  ...
  def koala_facebook_oauth
    @oauth = Koala::Facebook::OAuth.new(ENV['FACEBOOK_APP_ID'], ENV['FACEBOOK_APP_SECRET'], 'http://www.ebentum.com/users/auth/facebook/callback')
  end
